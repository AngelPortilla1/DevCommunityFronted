...
export class TrendingPage implements OnInit {
  ...
  setTab(tab: 'hoy' | 'semana' | 'mes'): void {
  ...
  loadPosts(): void {
    ...
    let sinceHours = 24;
    if (this.currentTab === 'semana') sinceHours = 168;
    if (this.currentTab === 'mes') sinceHours = 720;

    this.apiService.getPosts(this.page, this.limit, undefined, 'most_liked', sinceHours).subscribe({
      next: (res) => {
